...
from gear_sonic_sim.metric_utils import check_contact, check_height
from gear_sonic_sim.sim_utils import get_subtree_body_names
from gear_sonic_sim.unitree_sdk2py_bridge import ElasticBand, UnitreeSdk2Bridge
from gear_sonic_sim.robot import Robot

# VENDOR PATCH: ROBOT_SCENE is resolved against this root. In the bundle the
# models live at sim/models/, so the root is sim/ -- two levels up, not four.
GEAR_SONIC_ROOT = Path(__file__).resolve().parent.parent
...
class DefaultEnv:
    ...
    def start_image_publish_subprocess(self, start_method: str = "spawn", camera_port: int = 5555):
        from gear_sonic_sim.image_publish_utils import ImagePublishProcess
    ...
    def init_scene(self):
        ...
        # VENDOR PATCH: sim_step() and handle_keyboard_button() both read
        # self.elastic_band unconditionally, but it was only ever assigned inside
        # the branch below. The vendor's own config sets ENABLE_ELASTIC_BAND: True
        # so they never reach the else. Running without the band raises
        # AttributeError on the first physics step.
